...
- [x] `pages/sections/prompt-snippets/multi-angle-expansion.md`: '여러 사고법으로 아이디어 확장하기' 단일 통합 페이지 구축 (문제 재정의 5각도 + 이종 분야 결합 + 2단계 종합 확장형 3개 카드 통합, 초보자용 직관적 한국어 문체, AI 티 완전 제거)
- [x] `data/page-registry.json` & `data/navigation.json`: `prompt-snippets-multi-angle-expansion` 1개 단일 라우트로 등록 및 사이드바 내비게이션 정돈 동기화
- [x] 빌드(75개 페이지), 프롬프트 무결성 감사(75개 페이지), 단위 테스트(77개) 100% 정상 통과

## 3. Verification & Results
- **'여러 사고법으로 아이디어 확장하기' 단일 통합 메뉴 구축**:
  - 기존의 개별 메뉴 분리 방식을 지양하고, 사이드바 정보 밀도 최적화를 위해 **1개 단일 종합 메뉴**(`/prompt-snippets/multi-angle-expansion/`)로 깔끔하게 통합.
  - 카드 1: `문제 다시 정의하기` (반대, 범위, 상하위, 시간축, 타분야 5각도 비틀기)
  - 카드 2: `다른 분야에서 아이디어 빌려오기` (전혀 다른 이종 도메인의 성공 원리 차용 3단계)
  - 카드 3: `종합 다각도 확장 프롬프트` (문제 재정의 ➔ 이종 결합 ➔ 구체적 실행 대안 2단계 실전 완성형)
- **자연스러운 한글 문체 (AI 티 완전 제거)**:
  - 번역투(~에 대하여, ~를 통해, ~할 수 있다), 기계적 대구, 형식명사(~인 것이다)를 배제하고 사람이 직접 설명하듯 단정하고 리듬감 있는 구어/문어체로 작성.
  - 수식화된 가짜 정밀성(Pseudo-Math)과 글자수 강제를 걷어내고, 초보자가 바로 복사해 쓸 수 있는 직관적 3단계 출력 구조 적용.
- **프롬프트 한스푼 전 섹션 독립 카드화**: `언제 사용하나요?` / `1. 문제 재정의` / `2. 이종 분야 결합` / `3. 종합 확장` / `함께 사용하면 좋은 프롬프트` / `💡 AI 활용 TIP` 6개 영역이 각각 `---`로 완벽히 분리되어 독립 카드 박스로 렌더링됨.
- **제목-프롬프트 간 은은한 구분선 복원**: 대표/추가 프롬프트 카드 내부에서 상단 제목·설명문과 하단 프롬프트 본문 사이를 가볍게 나누어주는 1px 은은한 구분선 복원 완료.
- **출처(Source) 독립 분리**: 본문 프롬프트 박스 하단 [프롬프트 복사] 버튼 아래에서 카드 박스가 완전히 닫히고, 그 바깥에 `Source : Threads (@7goldpig7)` 박스가 별도의 콤팩트 라운딩 박스로 완벽히 분리 렌더링됨.
