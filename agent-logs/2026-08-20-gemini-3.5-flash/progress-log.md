...
7. **"인물을 엉뚱한 낙서 캐릭터로 바꾸기" 추가**
   ...
   - **silly-doodle 옵션 리스트 내부 개행 클리닝**: `silly-doodle.md` 파일에서 프롬프트 원본의 내용 및 단어를 100% 보존한 상태로, 옵션 상세 설명의 리스트 아이템 간 불필요한 개행(`\n`)을 기계적으로 제거하여 가독성을 높이고 AI 마크다운 파싱 구조를 단일화 완료 (원격 깃허브 main 브랜치 커밋 및 푸시 완료).
