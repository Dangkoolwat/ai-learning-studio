...
- **메타데이터(preview/source) 필수 기재 및 검증 규칙**: 새로 작성하거나 정돈하는 모든 프롬프트 파일의 frontmatter에는 `preview`와 `source` 키의 규격 적합성(비어있지 않음)을 확인해야 하며, `audit_prompts.py` 진단 도구를 구동해 누락 및 오작동 여부를 지속 확인해야 합니다.
