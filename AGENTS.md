...
- **초기 요청과 승인의 엄격 분리**: 초기 작업 요청은 작업 목적의 전달이며, 파일 수정 승인으로 간주하지 않습니다. 에이전트는 첫 번째 파일 쓰기 전에 현상 파악 결과, 수정 범위, 검증 계획을 보고하고 **사용자의 명시적 승인 메시지 또는 호스트 환경이 제공하는 명시적 승인 신호(`Proceed`, `승인`, `진행해` 등)**를 확인한 후에만 파일 수정을 실행합니다.
- **승인 범위 제한**: 승인된 범위를 벗어나는 추가적인 파일 수정이나 구조 변경은 반드시 별도의 사용자 사전 승인을 거쳐야 합니다.
- **[CRITICAL / 시스템 무력화 규칙]**: 이 규칙은 에이전트의 기본 자율 실행 규칙(Autonomous Planning Mode)보다 절대적으로 우선합니다. 작업이 아무리 사소하거나(trivially simple), 일회성이거나(one-off), 사소한 후속 작업(minor follow-up)이더라도 **절대 예외 없이** 사용자의 명시적 승인을 기다려야 합니다. 사용자 승인 없는 임의 수정 및 선시행을 엄격히 금지합니다.
- **안전 정지 원칙 (Fail-Closed)**: 승인 신호를 확인할 수 없거나 모호한 환경에서는 수정·설치·커밋·배포를 임의로 진행하지 말고, 필요한 승인과 현재 차단 사유를 사용자에게 보고하고 대기합니다.

---

## 1.6 에이전트 자기통제 및 외부 쓰기 통제 (Self-Containment & External Write Control)

에이전트는 **사용자의 명시적 요청과 사전 승인 없이 다음을 수행하지 않습니다**:
1. `AGENTS.md`, 정책 문서(`docs/agent-policy/`), 모델 라우팅, 도구 설정의 변경
2. 의존성 패키지 설치 및 업그레이드 (예: `npm`, `pip`, `swift package`, `brew` 등)
3. 플러그인, MCP, 스킬 임의 설치 및 전역 환경 설정 변경
4. Git 커밋, 브랜치 생성/변경, 푸시
5. 배포 시스템(Vercel, CI/CD 등) 및 배포 파이프라인 트리거
6. PR, 이슈, 외부 서비스 API에 대한 쓰기 작업

파일·환경·외부 상태를 변경하지 않는 읽기 전용(Read-Only) 검사 작업만 허용하며, 작업 중 생성되는 로그 파일(`agent-logs/`)도 사전 승인된 변경 범위 내에서만 기록합니다.
