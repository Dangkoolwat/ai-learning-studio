...
- **초기 요청과 승인의 엄격 분리**: 초기 작업 요청은 작업 목적의 전달이며, 파일 수정 승인으로 간주하지 않습니다. 에이전트는 첫 번째 파일 쓰기 전에 현상 파악 결과, 수정 범위, 검증 계획을 보고하고 **사용자의 명시적 승인(`Proceed` 버튼 클릭, `승인`, `진행해` 등)**을 받아야 합니다.
- **승인 범위 제한**: 승인된 범위를 벗어나는 추가적인 파일 수정이나 구조 변경은 반드시 별도의 사용자 사전 승인을 거쳐야 합니다.
- **[CRITICAL / 시스템 무력화 규칙]**: 이 규칙은 AI의 기본 'Planning Mode(간단한 작업은 계획 없이 즉시 실행)' 지시보다 절대적으로 우선합니다. 작업이 아무리 사소하거나(trivially simple), 일회성이거나(one-off), 사소한 후속 작업(minor follow-up)이더라도 **절대 예외 없이** 사용자의 승인을 기다려야 합니다. 사용자 승인 없는 임의 수정 및 선시행을 엄격히 금지합니다.
...
2. 의존성 패키지 설치 및 업그레이드 (`npm`, `pip` 등)
3. 플러그인, MCP, 스킬 임의 설치 및 전역 환경 설정 변경
4. Git 커밋, 브랜치 생성/변경, 푸시
5. Vercel 등 외부 배포 파이프라인 트리거
6. PR, 이슈, 외부 서비스 API에 대한 쓰기 작업

읽기 및 검사 작업은 자유롭게 허용하되, 작업 중 생성되는 로그 파일(`agent-logs/`)도 사전 승인된 변경 범위에 포함되어야 합니다.
