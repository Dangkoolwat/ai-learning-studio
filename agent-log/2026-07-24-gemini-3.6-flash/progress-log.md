...
- `core/build_pipeline.py`, `core/component_models.py`, `core/component_registry.py`, `core/component_engine.py`, `components/prompt-item.html`, `core/renderers/static_prompt.py`, `assets/css/site.css`:
  - `source` front matter 속성 지원 및 프롬프트 카드 맨 하단 소프트 라운딩 박스 기반의 출처 표시 컴포넌트(`prompt-item__source`) 구현 완료 (예: `Source : @Thread 김백곰 제공`).

## 3. 검증 결과
- `python3 scripts/build.py` 정적 사이트 빌드 정상 완료 (Pages: 12, Assets: 10, Routes: 12)
- `dist/ready-to-use/email/index.html` 출처 표기 라운딩 박스 렌더링 확인 완료
